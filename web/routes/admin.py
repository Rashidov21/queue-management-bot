...
async def admin_dashboard(request: Request):
    ...
    recent_activity = get_recent_activity_html()
...
def get_dashboard_stats():
    ...
    try:
        ...
        # Active providers
        active_providers = db.query(Provider).filter(Provider.is_accepting == True).count()
        
        # Today's bookings
        today_bookings = db.query(Booking).join(Slot).filter(
            and_(Slot.date == today, Booking.status == "active")
        ).count()
...
def get_recent_activity_html():
    """Get recent activity HTML"""
    db = SessionLocal()
    try:
        # Get recent bookings
        recent_bookings = db.query(Booking, Slot, Provider, User, Service)\
            .join(Slot, Booking.slot_id == Slot.id)\
            .join(Provider, Slot.provider_id == Provider.id)\
            .join(User, Booking.client_id == User.id)\
            .join(Service, Provider.service_id == Service.id)\
